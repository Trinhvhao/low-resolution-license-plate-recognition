"""
Phase 11 — Micro fine-tune from Phase 10 (79.33%) with STABLE LR schedule.

╔═══════════════════════════════════════════════════════════════════════════╗
║  ROOT CAUSE ANALYSIS — Phase 10 oscillation problem:                     ║
║                                                                           ║
║  Phase 10 used OneCycleLR with pct_start=0.3:                            ║
║  - At epoch 5, backbone unfroze → scheduler REBUILT for 55 epochs        ║
║  - Peak LR at 30% of 55 = epoch ~21: head=4e-4, backbone=2e-5           ║
║  - Epoch 21: accuracy CRASHED 79.33% → 76.83%!                          ║
║  - Model never fully recovered despite LR decay afterwards               ║
║                                                                           ║
║  FIX: CosineAnnealingLR — pure monotonic decay, NO peaks, NO restarts   ║
║  Start LR already low (4x lower) and ONLY goes down                      ║
╚═══════════════════════════════════════════════════════════════════════════╝

Strategy:
  1. Start from Phase 10 best (79.33%, epoch 12) — current best model
  2. CosineAnnealingLR — monotonic cosine decay, no peaks
  3. Very conservative LR: backbone=5e-6, lstm=5e-5, head=1e-4
     (4x lower than Phase 10 peak that caused the crash)
  4. All parameters trainable from start (no progressive unfreezing)
  5. SWA from epoch 12, every 3 epochs — more snapshots near convergence
  6. Enhanced TTA with 6 views (was 4 in Phase 10)
  7. 40 epochs (micro-tuning doesn't need 60)
"""
...
CFG = {
    'epochs':                40,        # shorter — micro fine-tuning
    'batch_size':            24,
    'gradient_accumulation': 4,         # eff batch = 96

    # Discriminative LR — VERY conservative (4x lower than P10 peak)
    'lr_backbone':           5e-6,      # P10 peak was 2e-5
    'lr_lstm':               5e-5,      # P10 peak was 2e-4
    'lr_head':               1e-4,      # P10 peak was 4e-4
    'lr_min':                1e-7,      # lower floor for finer convergence
    'weight_decay':          1e-4,
    'max_grad_norm':         1.0,
    ...
    'hidden_size':           256,
    'num_lstm_layers':       2,
    'dropout':               0.3,       # slightly higher than P10 (0.25)
    ...
    # Early stopping
    'patience':              25,        # shorter — micro-tuning converges fast

    # Regularization
    'label_smoothing':       0.03,      # slightly lower (P10 was 0.05)

    # SWA config — collect more snapshots near convergence
    'swa_start_epoch':       12,        # start earlier (model already near optimal)
    'swa_interval':          3,         # every 3 epochs (was 5 in P10)

    # Enhanced TTA — 6 views (was 4 in P10)
    'use_tta':               True,
    'tta_n_augs':            6,
    ...
    # Save
    'model_save_name':       'checkpoints/best_model_phase11.pth',
    'swa_model_save_name':   'checkpoints/best_model_phase11_swa.pth',
}


# ════════════════════════════════════════════════════════════════════════════
# Augmentation — Conservative (model is near-optimal)
# ════════════════════════════════════════════════════════════════════════════
def get_train_transforms():
    """Conservative augmentation — model is very close to optimal."""
    return A.Compose([
        A.Resize(height=IMG_HEIGHT, width=IMG_WIDTH),
        # Gentle geometric
        A.Affine(scale=(0.93, 1.07), translate_percent=(0.05, 0.05),
                 rotate=(-5, 5), shear=(-3, 3), p=0.5, fill=128),
        A.Perspective(scale=(0.02, 0.05), p=0.25),
        # Color
        A.RandomBrightnessContrast(brightness_limit=0.2, contrast_limit=0.2, p=0.5),
        A.HueSaturationValue(hue_shift_limit=8, sat_shift_limit=15,
                             val_shift_limit=15, p=0.25),
        A.OneOf([
            A.CLAHE(clip_limit=2.5, p=1.0),
            A.Equalize(p=1.0),
        ], p=0.15),
        # Blur/Noise — lighter
        A.OneOf([
        ...
        # Occlusion
        A.CoarseDropout(num_holes_range=(1, 2), hole_height_range=(3, 10),
                        hole_width_range=(4, 14), fill=128, p=0.25),
        A.ImageCompression(quality_range=(35, 75), p=0.15),
        # Normalize
...
def get_tta_transforms():
    """Enhanced TTA — 6 views for better inference (P10 used 4)."""
    return [
        ...
        # 5. CLAHE (histogram equalization — helps with low contrast images)
        A.Compose([
            A.Resize(height=IMG_HEIGHT, width=IMG_WIDTH),
...
def get_param_groups(model, cfg):
    ...
    All params trainable from start (no progressive unfreezing).
    """
    backbone_params = []
    lstm_params = []
    head_params = []
...
# ════════════════════════════════════════════════════════════════════════════
# Test-Time Augmentation (Enhanced — 6 views)
# ════════════════════════════════════════════════════════════════════════════
@torch.no_grad()
...
def train_one_epoch(model, loader, ctc_criterion, sr_criterion,
    ...
    """Train one epoch. Note: NO scheduler step inside — scheduler steps per epoch."""
    model.train()
...
def validate_tta(model, val_dataset, device, tta_transforms):
    """Validate with Test-Time Augmentation (6 views)."""
    model.eval()
...
def main():
    ...
    log_dir = os.path.join(Config.LOG_DIR, f'phase11_{ts}')
    writer = SummaryWriter(log_dir)

    print("=" * 80)
    print("  PHASE 11 — Micro fine-tune from P10 (79.33%)")
    print("  FIX: CosineAnnealingLR (monotonic decay, NO peaks)")
    print("=" * 80)
    ...
    # ── Data ─────────────────────────────────────────────────────────────
    train_ds = Phase11Dataset(Config.DATA_ROOT, mode='train', split_ratio=0.8,
                              sr_target_h=cfg['sr_target_h'],
                              sr_target_w=cfg['sr_target_w'])
    val_ds = Phase11Dataset(Config.DATA_ROOT, mode='val', split_ratio=0.8,
                            sr_target_h=cfg['sr_target_h'],
                            sr_target_w=cfg['sr_target_w'])

    train_loader = DataLoader(train_ds, batch_size=cfg['batch_size'],
                              shuffle=True,
                              collate_fn=Phase11Dataset.collate_fn,
                              num_workers=cfg['num_workers'], pin_memory=True,
                              persistent_workers=True, drop_last=True)
    val_loader = DataLoader(val_ds, batch_size=cfg['batch_size'],
                            shuffle=False, collate_fn=Phase11Dataset.collate_fn,
                            num_workers=cfg['num_workers'], pin_memory=True,
    ...
    # ── CosineAnnealingLR — PURE MONOTONIC DECAY ────────────────────────
    # Key fix: NO peaks like OneCycleLR, NO restarts like CosineWarmRestart
    # LR starts at max and smoothly decays to eta_min over T_max epochs
    # η(t) = η_min + (η_max - η_min) * (1 + cos(πt/T_max)) / 2
    ...
    print(f"\n{'='*80}")
    print("  Starting Phase 11 micro fine-tuning from Phase 10...")
    ...
    print(f"  Scheduler: CosineAnnealingLR (monotonic decay → {cfg['lr_min']:.1e})")
    print(f"  NO peaks, NO restarts, NO progressive unfreezing")
    ...
    for epoch in range(cfg['epochs']):
        ...
        # Step scheduler ONCE per epoch (CosineAnnealingLR is per-epoch)
        scheduler.step()
    ...
    # ════════════════════════════════════════════════════════════════════
    # TTA Evaluation (Enhanced — 6 views)
    # ════════════════════════════════════════════════════════════════════
    if cfg['use_tta']:
        print(f"\n{'='*80}")
    ...
    # ── Summary ──────────────────────────────────────────────────────────
    print(f"\n{'='*80}")
    print("  Phase 11 Training Completed")
    print(f"{'='*80}")
    ...
    if p10_acc > 0:
        ...
        print(f"\n  Phase 10 -> Phase 11: {p10_acc:.2f}% -> {best_acc:.2f}% "
              f"({delta:+.2f}%)")
